...
public class MousePoles {
	...
	public static class ObjectPole extends Pole {
		...
		private enum RotateMode {
			DUAL_AXIS, BIAXIAL, SPIN
		}

		private Vector3[] axisVectors = { new Vector3(1, 0, 0), new Vector3(0, 1, 0), new Vector3(0, 0, 1) };
	...
	public static class ViewPole extends ViewProvider {
		private enum TargetOffsetDir {
			DIR_UP, DIR_DOWN, DIR_FORWARD, DIR_BACKWARD, DIR_RIGHT, DIR_LEFT
		}

		private enum RotateMode {
			DUAL_AXIS_ROTATE, BIAXIAL_ROTATE, XZ_AXIS_ROTATE, Y_AXIS_ROTATE, SPIN_VIEW_AXIS
		}

		private Vector3[] offsets = { new Vector3(0, 1, 0), new Vector3(0, -1, 0), new Vector3(0, 0, -1), new Vector3(0, 0, 1),
		...
		public void mouseClick(MouseButton button, boolean isPressed, MouseModifier modifiers, Vector2 position) {
			if (isPressed) {
				...
				if (isDragging) {
					if (button == actionButton) {
						if (rotateMode == RotateMode.DUAL_AXIS_ROTATE || rotateMode == RotateMode.SPIN_VIEW_AXIS
